...
class QuantumWeatherSystem:
    ...
    def update(self, dt: float, player_is_outdoors: bool):
        ...
        if self.superposition_storm_active:
            ...
            if self.storm_collapse_timer <= 0 or player_is_outdoors:
                self.collapse_storm()
        
        # Low chance to start a new superposition storm
        if not self.superposition_storm_active and random.random() < 0.001:
    ...
    def start_superposition_storm(self):
        ...
        self.storm_collapse_timer = 30.0 # Will collapse on its own after 30s

    def collapse_storm(self):
        """Collapses the storm into a single, definite weather state."""
        final_state = random.choice(["sunny", "rainy"])
